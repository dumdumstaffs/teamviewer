        ...
        <Layout mode="auth">
            {!account ? null : (
                <div className="px-4 md:px-10 xl:px-20 py-12 dark:bg-neutral-900">
                    <h3 className="text-2xl mb-6">Account: #{account.id.toUpperCase()}</h3>

                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                        {account.stocks.map((stock, i) => (
                    ...
                    <div className="mt-12">
                        ...
                        <p className="text-3xl mb-1">{account.balance().toLocaleString()} USD</p>
                        <p className="mb-3 flex items-center space-x-6">
                            <span className="text-sm whitespace-nowrap">Account Holder(s):</span>
                            <span className="text-sm font-medium">{account.holders.join(", ")}</span>
                        </p>

                        {/* assets */}
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 py-4">
                            {account.stocks.map((stock) => (
                                <AssetCard key={stock.symbol} name={stock.name} icon={stock.icon} price={stock.price} />
                            ))}
                        </div>
                    ...
                    <div className="w-full h-96 rounded-md overflow-hidden mt-12">
                        <SymbolOverview autosize isTransparent symbols={account.stocks.map((stock) => [stock.name, stock.overview])} />
                    </div>
                </div>
            )}
        </Layout>
